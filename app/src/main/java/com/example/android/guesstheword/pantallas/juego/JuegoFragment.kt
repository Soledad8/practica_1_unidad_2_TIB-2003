...
lateinit var vistaModelo : JuegoViewModel
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        vistaModelo.palabra.observe(viewLifecycleOwner, Observer {
                nuevaPalabra -> binding.palabraText.text = nuevaPalabra

        })
        vistaModelo.puntuacion.observe(viewLifecycleOwner, Observer {
                nuevaPuntuacion -> binding.puntuacionText.text = nuevaPuntuacion.toString()

        })
        vistaModelo.elEventoTermino.observe(viewLifecycleOwner, Observer {
                terminoElJuego ->
            if (terminoElJuego){
                juegoTerminado()
                vistaModelo.elJuegoHaTerminado()
            }
